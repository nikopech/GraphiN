...
excludeFromGraph<-function(data,...)
{
...
data=includeInGraph(dataset=data,Summary.D.GENE.and.allele=c("Homsap TRBD1*01 F","Homsap TRBD2*01 F"),Summary.J.GENE.and.allele="Homsap TRBJ1-2*01 F")

data=excludeFromGraph(dataset=data,Summary.D.GENE.and.allele=c("Homsap TRBD1*01 F","Homsap TRBD2*01 F"))
data=read.csv("filterInTableT288.txt",sep="",header = TRUE)
