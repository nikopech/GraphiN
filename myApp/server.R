...
shinyServer(function(input,output,session){
  ...
  shinyFileChoose(input,"files",roots=c(home = '.'),filetypes=c('',"txt","csv"))
  
  fulldata <- eventReactive(input$files,{
    file1<-parseFilePaths(roots=c(home = '.'),input$files)
    #file1 <- input$file
  ...
  output$downloadMST <- downloadHandler(filename = function() {
        paste("mst.csv")
      }, content = function(file) {
  ...
  output$downloadCentral <- downloadHandler(filename = function() {
  ...
  output$downloadCluster <- downloadHandler(filename = function() {
